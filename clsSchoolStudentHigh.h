...
class clsSchoolStudentHigh : public clsPerson
{
...
private:
	...
	static clsSchoolStudentHigh _GetEmptyStudentObject()
	{
		return clsSchoolStudentHigh(enMode::EmptyMode, "", "", 0, "", "", "", 0, 0, "");
	}

	static clsSchoolStudentHigh _ConvertLineToStudentObject(string Line, string Seperator = "#//#")
	{
		vector <string> vStudent = clsString::Split(Line, Seperator);

		return clsSchoolStudentHigh(enMode::UpdateMode, vStudent[0],
			vStudent[1], stof(vStudent[2]), vStudent[3], vStudent[4], vStudent[5],
			stof(vStudent[6]), stof(vStudent[7]), vStudent[8]);
	}

	static string _ConverStudentsObjectToLine(clsSchoolStudentHigh Student, string Seperator = "#//#")
	{
		string stDataLine = "";
	...
	static vector<clsSchoolStudentHigh> _LoadStudentsDataFromFile()
	{
		vector<clsSchoolStudentHigh> _vStudents;

		fstream MyFile;
		MyFile.open("StudentHigh.txt", ios::in);
		...
		if (MyFile.is_open())
		{
			...
			{
				clsSchoolStudentHigh Students = _ConvertLineToStudentObject(Line);
	...
	static void _SaveStudentsDataToFile(vector <clsSchoolStudentHigh> vStudent)
	{
		string DataLine;

		fstream MyFile;
		MyFile.open("StudentHigh.txt", ios::out);

		if (MyFile.is_open())
		{
			for (clsSchoolStudentHigh& S : vStudent)
			{
	...
	void _Update()
	{
		vector<clsSchoolStudentHigh> _vStudent;
		_vStudent = _LoadStudentsDataFromFile();

		for (clsSchoolStudentHigh& S : _vStudent)
		{
	...
	void _AddNewLineToFile(string Line)
	{
		...
		MyFile.open("StudentHigh.txt", ios::out | ios::app);
...
public:
	...
	clsSchoolStudentHigh(enMode Mode, string FirstName, string LastName, short Age, string Email, string Phone,
	...
	static clsSchoolStudentHigh Find(string AccountNumberOfStudent)
	{
		fstream MyFile;
		MyFile.open("StudentHigh.txt", ios::in);
		...
		if (MyFile.is_open())
		{
			...
			while (getline(MyFile, Line))
			{
				clsSchoolStudentHigh Student = _ConvertLineToStudentObject(Line);
	...
	{
		clsSchoolStudentHigh Student = Find(AccountNumberOfStudent);
	...
	enSaveResults Save()
	{
		...
		{
			if (clsSchoolStudentHigh::IsStudentExist(_AccountNumberOfStudent))
			{
	...
	static clsSchoolStudentHigh CreateNewStudentObject(string AccountNumberOfStudent)
	{
		return clsSchoolStudentHigh(enMode::AddNewMode, "", "", 0, "", "", AccountNumberOfStudent,
			0, 0, "");
	}

	bool Delete()
	{
		vector<clsSchoolStudentHigh> vStudents = _LoadStudentsDataFromFile();
	...
	static vector<clsSchoolStudentHigh> GetStudentList()
	{
	...
	{
		vector<clsSchoolStudentHigh> vStudents = GetStudentList();
