...
class clsSchoolStudentJuniorHigh : public clsPerson
{
...
private:
	...
	enMode _Mode;
	string _Classroom = "Junior High School";
	...
	static clsSchoolStudentJuniorHigh _GetEmptyStudentObject()
	{
		return clsSchoolStudentJuniorHigh(enMode::EmptyMode, "", "", 0, "", "", "", 0, 0, "");
	}

	static clsSchoolStudentJuniorHigh _ConvertLineToStudentObject(string Line, string Seperator = "#//#")
	{
		vector <string> vStudent = clsString::Split(Line, Seperator);

		return clsSchoolStudentJuniorHigh(enMode::UpdateMode, vStudent[0],
			vStudent[1], stof(vStudent[2]), vStudent[3], vStudent[4], vStudent[5],
			stof(vStudent[6]), stof(vStudent[7]), vStudent[8]);
	}

	static string _ConverStudentsObjectToLine(clsSchoolStudentJuniorHigh Student, string Seperator = "#//#")
	{
		string stDataLine = "";
	...
	static vector<clsSchoolStudentJuniorHigh> _LoadStudentsDataFromFile()
	{
		vector<clsSchoolStudentJuniorHigh> _vStudents;

		fstream MyFile;
		MyFile.open("StudentJuniorHigh.txt", ios::in);
		...
		if (MyFile.is_open())
		{
			...
			{
				clsSchoolStudentJuniorHigh Students = _ConvertLineToStudentObject(Line);
	...
	static void _SaveStudentsDataToFile(vector <clsSchoolStudentJuniorHigh> vStudent)
	{
		string DataLine;

		fstream MyFile;
		MyFile.open("StudentJuniorHigh.txt", ios::out);

		if (MyFile.is_open())
		{
			for (clsSchoolStudentJuniorHigh& S : vStudent)
			{
	...
	void _Update()
	{
		vector<clsSchoolStudentJuniorHigh> _vStudent;
		_vStudent = _LoadStudentsDataFromFile();

		for (clsSchoolStudentJuniorHigh& S : _vStudent)
		{
	...
	void _AddNewLineToFile(string Line)
	{
		...
		MyFile.open("StudentJuniorHigh.txt", ios::out | ios::app);
...
public:
	...
	clsSchoolStudentJuniorHigh(enMode Mode, string FirstName, string LastName, short Age, string Email, string Phone,
	...
	static clsSchoolStudentJuniorHigh Find(string AccountNumberOfStudent)
	{
		fstream MyFile;
		MyFile.open("StudentJuniorHigh.txt", ios::in);
		...
		if (MyFile.is_open())
		{
			...
			while (getline(MyFile, Line))
			{
				clsSchoolStudentJuniorHigh Student = _ConvertLineToStudentObject(Line);
	...
	{
		clsSchoolStudentJuniorHigh Student = Find(AccountNumberOfStudent);
	...
	enSaveResults Save()
	{
		...
		{
			if (clsSchoolStudentJuniorHigh::IsStudentExist(_AccountNumberOfStudent))
			{
	...
	static clsSchoolStudentJuniorHigh CreateNewStudentObject(string AccountNumberOfStudent)
	{
		return clsSchoolStudentJuniorHigh(enMode::AddNewMode, "", "", 0, "", "", AccountNumberOfStudent,
			0, 0, "");
	}

	bool Delete()
	{
		vector<clsSchoolStudentJuniorHigh> vStudents = _LoadStudentsDataFromFile();
	...
	static vector<clsSchoolStudentJuniorHigh> GetStudentList()
	{
	...
	{
		vector<clsSchoolStudentJuniorHigh> vStudents = GetStudentList();
